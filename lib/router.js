...
function parseEndPoint(id, endPoint) {
	...
	// check for reroute
	var route = endPoint !== '' && res.controller === null && res.method === null ? '/' : '/' + res.controller + '/' + res.method;
	if (reroutes[route]) {
		var params = '/' + res.params.join('/');
		logger.verbose('reroute found [', route, ']: rerouting to [', reroutes[route] + params, ']');
		return parseEndPoint(id, reroutes[route] + params);
	}
	return res;
}
